...
Podendo ordenar os resultados em ordem crescente (ASC - Padrão da ordenação) e 
...
(sum, count, avg, etc) para calculcar valores agregados para cada grupo.*/
...
/*Condição em Grupos (having) - É usado para filtrar os resultados que usa GROUP By
...
select categorias.nome as "Categoria", count(produtos.id) as "Total Produtos"
from produtos
join categorias on produtos.categoria_id = categorias.id
group by categorias.nome
...
menores, geralmente para melhorar a performance e a usabilidade em interface. Em
